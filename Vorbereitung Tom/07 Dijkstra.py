...
map3 = [
    ...
    [float('inf'), float('inf'), float('inf'), float('inf'), 3, 4, float('inf'), 0]
]
...
while eingabe >= 0:
    ...
    if eingabe == 4:
        ...
        while neighbourhoodKnots:
            predecessor = current
            current = min(neighbourhoodKnots, key=lambda k: neighbourhoodKnots[k][0]) # key of next element with minimum path length
        ...
        for key, value in explored.items():
            ...
            for i in range(len(via)):
                via[i] += 1
            print(destination,"\t\t",value[0],"\t\t",via,sep="")