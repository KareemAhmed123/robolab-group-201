...
class Deploy:
    ...
    def __init__(self, configure=False, execute_only=True, backup=False, sync_log=False, get_file=list,
                 join_session=True, exam=False):
        """
    ...
    def routine(self):
        ...
        try:
            ...
            # Enter if file fetching is requested
            if len(self.get_file):
                system.get_file(self.get_file)
                return
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-g', '--get-file', help='Fetch files from the brick', nargs='+')
    parser.add_argument(
    ...
    try:
        ...
        deploy = Deploy(args.configure, args.execute_only, args.backup, args.sync_log, args.get_file, not args.reload,
                        args.exam)
        deploy.routine()
