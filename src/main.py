...
def run():
    ...
    client = mqtt.Client(
        client_id=client_id,  # Unique Client-ID to recognize our program
    ...
    # IT MAY BE NEEDED TO
    mechanics = Mechanics()
    #mechanics.calibrate_blue()


    communication = Communication(client, logger)
    planet = Planet()
    integration = Integration(communication,planet, logger)
    ...
    integration.start_robot(communication, mechanics, planet)
    return
